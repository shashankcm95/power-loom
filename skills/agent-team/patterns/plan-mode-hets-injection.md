...
| Treat plan template as enforced schema without enforcement | Soft template in H.7.9; **enforcement live as of H.7.12** via `validators/validate-plan-schema.js` (tiered enforcement — Tier 1 mandatory, Tier 2 conditional on "Routing Decision" string, Tier 3 aspirational). **H.7.17 migration**: hook moved from PreToolUse:Edit\|Write (H.7.12 conservative path) to PostToolUse:Edit\|Write (theo's H.7.9 Section C original spec) after `claude-code-guide` confirmed PostToolUse:Write is supported by Claude Code. Forcing instruction now goes to stdout (matches error-critic.js PostToolUse pattern); hook can't block by design (PostToolUse fires after Write/Edit completes). |
