...
| 20 | [Research-Plan-Implement (RPI)](research-plan-implement.md) | active | H.8.6 adoption of canonical RPI workflow from [humanlayer/advanced-context-engineering-for-coding-agents](https://github.com/humanlayer/advanced-context-engineering-for-coding-agents). Three slash commands (`/research`, `/plan`, `/implement`) with persistent markdown artifacts in `swarm/thoughts/shared/{research,plans}/`. Documentary phase (research, no critique) separate from synthesis phase (plan, critic personas welcome) separate from execution phase (implement, phase-by-phase pause-for-verification). Maps to four-dimension context engineering (Correctness > Completeness > Size > Trajectory). NEW documentary persona contracts 14/15/16 forbid critique language at the contract layer. |
