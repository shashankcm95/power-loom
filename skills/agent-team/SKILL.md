...
- **H.3+ (next)**: see CS-1 super-root recommendations. Top items: (a) ship the 4 quick fixes bundle (`--skip-checks mustNotSkip`, `unknown_check` failure on required, `max_depth` enforcement, `hierarchical-aggregate.js` path), (b) extract shared `withLock` to `_lib/lock.js` + apply to 4 RMW scripts, (c) re-run install.sh + add post-phase install-sync convention, (d) add structural checks to 7 builder contracts, (e) fix the 29 contracts-validate violations.
